...
import random
import hashlib
from db import get_db, get_standalone_conn, init_db, init_pool, close_pool, DB_BACKEND, DB_PATH
import asyncio
import logging
import smtplib
import statistics
...
# ─── Database ─────────────────────────────────────────────────────────────────
# init_db and get_db are now in db.py
init_db()
...
def user_memory_bulk_visibility(
    ...
    # (documented pattern: _log_memory_access uses its own standalone connection)
    log_entries = []
    with get_db() as db:
        _verify_agent_ownership(db, agent_id, user_id)
